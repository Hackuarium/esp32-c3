...
void TaskBatteryLevel(void* pvParameters) {
  ...
  while (true) {
    ...
    // A voltage divider is used with the following connections: BAT+/VBUS -> R1 -> GPIO0 and GND/BAT- -> R2 -> GPIO0 with R1=R2=220kOhm 
    // A first test indicates that a full NCR18650B battery (4.12V) gives a value of 2895, and empty (2.615V) gives a value of 1869.
    // The discharge of the NCR18650B with WiFi connected without sleep lasted 39h42min. The discharge decreased almost linearly except
    // for the last 2h (from value 2281 until 1869) where the voltage drop is faster.
    vTaskDelay(100);
