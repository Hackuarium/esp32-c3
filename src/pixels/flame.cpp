...
void updateFlame(Adafruit_NeoPixel& pixels) {
  if (flameCounter > 0)
    flameCounter--;
  ...
  switch (flameStatus) {
    case 0:  // normal intensity
      ...
      if (flameCounter == 0 &&
          random(0, (8 - getParameter(PARAM_INTENSITY)) * 50) ==
              0) {  // how often there is a flash
    ...
    case 3:  // damping 3s, 14 oscillation
      ...
      int16_t extraIntensity =
          (medianIntensity - minIntensity) * flameCounter / 75;
  ...
  flameColor =
      getColor(getParameter(PARAM_COLOR_MODEL), getHSVSpeedChange(), intensity);
  for (int led = 0; led < MAX_LED; led++) {
    pixels.setPixelColor(led, flameColor);
  }
